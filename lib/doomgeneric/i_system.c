...
// M5StickC Plus has only 320KB RAM total!
// With calculated trig (no 64KB tables), we have more room
// Start with minimal and let malloc handle what it can
#define DEFAULT_RAM 0 /* MiB - use minimal, will be clamped in code below */
#define MIN_RAM     0  /* MiB */

// Override zone size - extreme reduction, rely on IRAM for large data
#define ESP32_ZONE_SIZE (32 * 1024)  // 32KB for zone memory
...
static byte *AutoAllocMemory(int *size, int default_ram, int min_ram)
{
    ...
    // ESP32 optimization: Fixed zone size for embedded system
    // Don't try to allocate MBs, use our predetermined size
    #ifdef ESP32_ZONE_SIZE
    *size = ESP32_ZONE_SIZE;
    zonemem = malloc(*size);
    if (zonemem == NULL)
    {
        I_Error("Unable to allocate %i KB of RAM for zone", *size / 1024);
    }
    return zonemem;
    #endif
